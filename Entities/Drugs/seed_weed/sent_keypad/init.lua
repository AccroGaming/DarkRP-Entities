...
local function RunKeypad(Repeats, Length, Delay, Key, Owner)
	for i = 0, Repeats do
		timer.Simple(Length*(i)+Delay*i, function () numpad.Activate(Owner, Key) end)
		timer.Simple(Length*(i+1)+Delay*i, function () numpad.Deactivate(Owner, Key) end)
	end
	...
	elseif (Args[2] == "accept") then
		...
		if (Key != nil) and (Key != -1) then
			if (Simple) then
				...
				timer.Simple(Length, function() numpad.Deactivate(Owner, Key) end)
			else
				if (InitDelay != 0) then
